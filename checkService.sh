...
licenseFile="/opt/adp-install/gurobi.lic.metaphor"
while [ 1 == 1 ]
do
	#English Check
	...
	if [ $time_diff -gt 86400 ]; then
		yest_date=`date "+%s"`
		EMAIL="/tmp/serviceEmail.txt"
		subject="CheckServiceScript for Aug-Eval is running! Number of days till license expiration is " /users/metaphor/get-days-to-expiry.sh $licenseFile 
		echo "$subject" > $EMAIL
